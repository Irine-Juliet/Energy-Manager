...
for i in range(6):
    ...
    print(f"   ✓ Created: {activity.name} (ID: {activity.id}, Time: {activity.activity_date.strftime('%I:%M %p')})")
...
print(f"   ✓ Created: {early_activity.name} (ID: {early_activity.id}, Time: {early_activity.activity_date.strftime('%I:%M %p')})")
...
print(f"Expected: 5 most recent activities by time (2 PM, 1 PM, 12 PM, 11 AM, 10 AM)")
...
if actual_hours == expected_hours:
    ...
    print(f"   ✅ Activities are ordered by activity_date (when they occurred)")
    print(f"   ✅ Breakfast (8 AM) and 9 AM activity correctly excluded (they're 6th & 7th oldest)")
else:
...
if is_descending:
    print(f"   ✅ PASS: Activities are sorted in descending order (newest first)")
else:
    print(f"   ❌ FAIL: Activities are NOT in descending order")
...
print(f"Expected: All 7 activities in descending order")
...
if all_dates == sorted(all_dates, reverse=True):
    print(f"   ✅ PASS: History activities in correct descending order")
else:
    print(f"   ❌ FAIL: History activities NOT in correct order")
...
if not breakfast_in_top5 and not nine_am_in_top5:
    ...
    print(f"   ✅ Homepage shows most recent 5 by occurrence time, not log time")
else:
...
all_tests_passed = (
    actual_hours == expected_hours and
    is_descending and
    all_today_activities.count() == 7 and
    not breakfast_in_top5 and
    not nine_am_in_top5
)
