...
def test_dashboard_hours_aggregation():
    ...
    today_activities = Activity.objects.filter(
        user=user, activity_date__gte=today_start, activity_date__lt=today_end
    )
    ...
    assert (
        hours_per_category["2"] == 3.0
    ), f"Expected 3.0 hours for level 2, got {hours_per_category['2']}"
    assert (
        hours_per_category["-1"] == 0.25
    ), f"Expected 0.25 hours for level -1, got {hours_per_category['-1']}"
    print(f"✅ PASS - Energy level 2: {hours_per_category['2']} hours (expected 3.0)")
    print(
        f"✅ PASS - Energy level -1: {hours_per_category['-1']} hours (expected 0.25)"
    )
    ...
    print(
        f"✅ PASS - Total for 3 activities (1hr each): {hours_per_category['2']} hours (expected 3.0)"
    )
    ...
    assert (
        hours_per_category["2"] == 2.0
    ), f"Expected 2.0 hours for level 2, got {hours_per_category['2']}"
    assert (
        hours_per_category["-2"] == 0.5
    ), f"Expected 0.5 hours for level -2, got {hours_per_category['-2']}"
    print(
        f"✅ PASS - Energy level 2 (9:00am, 120 min): {hours_per_category['2']} hours (expected 2.0)"
    )
    print(
        f"✅ PASS - Energy level -2 (9:30am, 30 min): {hours_per_category['-2']} hours (expected 0.5)"
    )
    ...
    assert (
        hours_per_category["-2"] == 0.07
    ), f"Expected 0.07 hours for level -2, got {hours_per_category['-2']}"
    print(
        f"✅ PASS - 4 minutes rounded: {hours_per_category['-2']} hours (expected 0.07)"
    )
    ...
    assert (
        hours_per_category["2"] == 1.0
    ), f"Expected 1.0 hours for level 2, got {hours_per_category['2']}"
    print(
        f"✅ PASS - Today: {hours_per_category['2']} hour, Yesterday activity ignored (expected 1.0)"
    )
