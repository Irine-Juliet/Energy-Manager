...
class TestAutocompleteActivities:
    ...
    def test_autocomplete_returns_suggestions(self, authenticated_client, user):
        ...
        Activity.objects.create(
            user=user, name="Meeting", energy_level=1, duration=60, activity_date=now
        )
        Activity.objects.create(
            user=user,
        ...
        Activity.objects.create(
            user=user, name="Lunch", energy_level=1, duration=60, activity_date=now
        )
        ...
        if response.get("Content-Type", "").startswith("application/json"):
            ...
            suggestion_names = [
                s if isinstance(s, str) else s.get("name", "") for s in suggestions
            ]
    ...
    def test_autocomplete_top_5_limit(self, authenticated_client, user):
        ...
        # Search for "activity"
        response = authenticated_client.get(
            reverse("autocomplete_activities"), {"q": "activity"}
        )
    ...
    def test_autocomplete_frequency_ordering(self, authenticated_client, user):
        ...
        for _ in range(2):
            Activity.objects.create(
                user=user, name="Meetup", energy_level=1, duration=60, activity_date=now
            )
        ...
        if response.get("Content-Type", "").startswith("application/json"):
            ...
            if suggestions:
                ...
                first_name = (
                    ...
                    else first_suggestion.get("name", "")
                )
    ...
    def test_autocomplete_empty_query(self, authenticated_client, user):
        ...
        response = authenticated_client.get(
            reverse("autocomplete_activities"), {"q": ""}
        )
    ...
    def test_autocomplete_user_isolation(
        ...
        # Search for "meeting"
        response = authenticated_client.get(
            reverse("autocomplete_activities"), {"q": "meeting"}
        )
